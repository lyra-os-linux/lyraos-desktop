...
try:
 ...
 cmd(['groupmod','-n','lyra-parental-fixture-off','lyra-parental-fixture'])
 try:
  test('missing-registry-denied')
  test('missing-registry-ordinary-also-denied',user='ordinaryuser')
 finally:cmd(['groupmod','-n','lyra-parental-fixture','lyra-parental-fixture-off'])
 ...
 cmd(['setenforce','0'])
 pathlib.Path('/root/parental-pam-guard-result.json').write_text(json.dumps({'kind':'dedicated-pam-session-fixture','full_account_protection':False,'checks':records},indent=2)+'\n')
