...
def check(name,expected,ordinary=False,extra_env=None):
    ...
    cmd+=[launcher]
    r=subprocess.run(cmd,capture_output=True,text=True,timeout=10,env={**os.environ,**(extra_env or {})})
...
try:
    ...
    for value in ['/tmp/untrusted-notify','/run/user/1002/systemd/notify','@untrusted-notify','']:
        check('notification-socket-refused:'+repr(value),'unexpected notification socket',extra_env={'NOTIFY_SOCKET':value})
