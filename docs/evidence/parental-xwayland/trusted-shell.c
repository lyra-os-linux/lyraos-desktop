...
int main(int argc, char **argv) {
     ...
     * Only validated locale data and the exact user-manager notification
     * socket survive from the caller. Session identity
    ...
    char notify[256];
    snprintf(notify, sizeof notify, "%s/systemd/notify", runtime);
    const char *notify_value = getenv("NOTIFY_SOCKET");
    int has_notify = notify_value != NULL;
    if (has_notify && strcmp(notify_value, notify))
        refuse("unexpected notification socket");
...
#define SET(k, v) do { if (setenv((k), (v), 1)) refuse("cannot set environment"); } while (0)
    ...
    if (has_notify) SET("NOTIFY_SOCKET", notify);
    char bus[256];
