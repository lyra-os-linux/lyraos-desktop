...
absent.extend(['/usr/libexec/lyra/parental-services-probe','/opt/lyra-parental-probe/settings-probe','/etc/dconf/profile/lyra-supervised-raw-test','/home/parentaltest/.config/dconf'])
assert all(not P(p).exists() for p in absent)
labels=run(['ls','-lZ','/usr/libexec/gsd-xsettings','/usr/libexec/dconf-service'])
