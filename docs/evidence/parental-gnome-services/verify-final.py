...
r=run(['rpm','-V','gdm','mutter','xwayland','gnome-shell','gnome-session-core','gnome-session-wayland','at-spi2-core','xprop','xkbcomp','gnome-settings-daemon','accountsservice']);assert r['rc']==0 and not r['stdout']
...
absent.append('/usr/libexec/lyra/parental-services-probe')
assert all(not P(p).exists() for p in absent)
labels=run(['ls','-lZ','/usr/libexec/gsd-xsettings'])
