...
try:
 ...
 pathlib.Path('/root/parental-execution-final.json').write_text(json.dumps(result,indent=2)+'\n')
finally:
 # Boot fixture remains permissive for preparation; each accepted run enforces.
 subprocess.run(['setenforce','0'],check=True)
 pathlib.Path('/root/parental-execution-final-progress.json').write_text(json.dumps(records,indent=2)+'\n')
