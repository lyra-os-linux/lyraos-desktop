...
for u,uid in uids.items():
 ...
 assert p.stdout.strip()in ('inactive','failed'),(u,p.stdout)
...
try:
 for u,uid in uids.items():
  ...
  run(u+'-bus',['runuser','-u',u,'--','env',f'XDG_RUNTIME_DIR=/run/user/{uid}',f'DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/{uid}/bus','busctl','--timeout=5s','--user','--no-pager','list'])
  run(u+'-journal',['journalctl','-b','-u',f'user@{uid}.service','-n','30','--no-pager'])
 run('kernel-denials',['journalctl','-k','-n','70','--no-pager'])
 ...
 pathlib.Path('/root/parental-services-result.json').write_text(json.dumps({'enforcing_during_tests':True,'full_account_protection':False,'checks':rows},indent=2)+'\n')
