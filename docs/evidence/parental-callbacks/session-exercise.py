...
'''
    ...
    with P('/root/trusted-shell-session.json').open('w') as log:
        subprocess.run(['python3','/root/restricted-shell-session.py'],check=True,stdout=log,timeout=150)
    import time
    time.sleep(1)
    a=subprocess.run(['ausearch','-m','AVC','-ts',audit_stamp,'--raw'],capture_output=True,text=True)
    P('/root/trusted-shell-avc.log').write_text(a.stdout)
finally:
