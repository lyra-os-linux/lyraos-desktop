...
try:
 ...
 run('kernel-denials',['journalctl','-k','-n','25','--no-pager'])
 ...
 pathlib.Path('/root/parental-systemd-keyring-result.json').write_text(json.dumps({'enforcing_during_tests':True,'full_account_protection':False,'checks':rows},indent=2)+'\n')
