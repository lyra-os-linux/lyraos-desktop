...
r=run(['rpm','-V','gdm','mutter','xwayland','gnome-shell','gnome-session-core','gnome-session-wayland','at-spi2-core','xprop','xkbcomp','gnome-settings-daemon','accountsservice','dconf','libdconf1']);assert r['rc']==0 and not r['stdout']
...
absent.extend(['/usr/libexec/lyra/parental-services-probe','/opt/lyra-parental-probe/settings-probe','/etc/dconf/profile/lyra-supervised-raw-test','/home/parentaltest/.config/dconf','/opt/lyra-parental-probe/a11y-settings-probe','/opt/lyra-parental-probe/accessible-app','/opt/lyra-parental-probe/accessible-client'])
assert all(not P(p).exists() for p in absent)
labels=run(['ls','-lZ','/usr/libexec/gsd-xsettings','/usr/libexec/dconf-service','/usr/libexec/gsd-a11y-settings'])
