...
def snapshot(paths, generated, cache_directory=None):
    guard()
    with (ROOT / 'lock').open('a') as lock:
        ...
        if cache_directory is not None:
            ...
            assert str(directory) == '/home/parentaltest/.cache/fontconfig'
            assert directory.is_dir() and not directory.is_symlink()
            info = directory.stat()
            children = list(directory.iterdir())
            assert all(p.is_file() and not p.is_symlink() for p in children)
            cache = dict(path=str(directory), names=[p.name for p in children],
                         mode=stat.S_IMODE(info.st_mode), uid=info.st_uid, gid=info.st_gid,
                         atime=info.st_atime_ns, mtime=info.st_mtime_ns,
                         label=base64.b64encode(os.getxattr(directory, 'security.selinux')).decode())
        ...
        manifest = dict(backup=str(backup), entries=entries, cache_directory=cache, audit_backlog_limit=audit_status['backlog_limit'],
...
def recover():
    ...
    with (ROOT / 'lock').open('a') as lock:
        ...
        run(['systemctl', 'stop', 'gdm', 'user@1003.service', 'systemd-localed.service'], timeout=90)
        run(['setenforce', '0'])
        run(['systemctl', 'stop', 'auditd'])
        run(['auditctl', '-b', manifest['audit_backlog_limit']])
        for service in SERVICES:
            assert run(['systemctl', 'is-active', service], check=False).stdout.strip() == 'inactive'
        cache = manifest.get('cache_directory')
        if cache:
            directory = Path(cache['path'])
            assert str(directory) == '/home/parentaltest/.cache/fontconfig'
            assert directory.is_dir() and not directory.is_symlink()
            for child in directory.iterdir():
                if child.name not in cache['names']:
                    # Never recurse or follow an unexpected cache symlink.
                    assert child.is_symlink() or child.is_file(), child
                    child.unlink()
        ...
        for name in manifest['directories']:
            ...
            if path.exists():
                path.rmdir()
        if cache:
            os.chown(directory, cache['uid'], cache['gid'])
            directory.chmod(cache['mode'])
            os.setxattr(directory, 'security.selinux', base64.b64decode(cache['label']))
            assert {p.name for p in directory.iterdir()} == set(cache['names'])
            os.utime(directory, ns=(cache['atime'], cache['mtime']))
            assert directory.stat().st_mtime_ns == cache['mtime']
        if MODULE.exists():
