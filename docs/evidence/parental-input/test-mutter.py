...
import pathlib,subprocess,tempfile,sys,json,shlex,os
...
int main(void) {
 ...
 /* A missing runtime path must refuse legacy copies without fd leaks. */
 char *runtime=g_strdup(g_getenv("XDG_RUNTIME_DIR"));
 g_unsetenv("XDG_RUNTIME_DIR");
 assert(meta_anonymous_file_open_fd(f,META_ANONYMOUS_FILE_MAPMODE_SHARED)==-1 && errno==ENOENT);
 assert(count_fds()==current);
 g_setenv("XDG_RUNTIME_DIR",runtime,TRUE);g_free(runtime);
 ...
 assert(fcntl(rw,F_GETFD)&FD_CLOEXEC);
 struct stat st;assert(fstat(rw,&st)==0 && st.st_nlink==0);
 char path[64],target[4096];snprintf(path,sizeof(path),"/proc/self/fd/%d",rw);
 ssize_t n=readlink(path,target,sizeof(target)-1);assert(n>0);target[n]=0;
 assert(strstr(target,g_getenv("XDG_RUNTIME_DIR"))==target);
 assert(strstr(target,"(deleted)"));
...
'''
 ...
 env=dict(os.environ,XDG_RUNTIME_DIR=str(p))
 r=subprocess.run([str(p/'test')],capture_output=True,text=True,check=True,env=env)
 ...
 upstream_result=subprocess.run([str(p/'upstream')],capture_output=True,text=True,check=True,env=env)
 (p/'config.h').write_text('#define HAVE_MKOSTEMP 1\n#define HAVE_POSIX_FALLOCATE 1\n')
 subprocess.run(['cc','-D_GNU_SOURCE','-std=gnu17','-Wall','-Wextra','-Werror','-I'+str(p),'-c',str(src),'-o',str(p/'fallback.o')]+flags,check=True)
 subprocess.run(['cc','-D_GNU_SOURCE','-std=gnu17','-I'+str(p),str(src),str(upstream),'-o',str(p/'fallback')]+flags,check=True)
 fallback=subprocess.run([str(p/'fallback')],capture_output=True,text=True,check=True,env=env)
 print(json.dumps(dict(status='passed',stdout=r.stdout,source=str(src),upstream_test=dict(path='src/tests/anonymous-file.c',rc=upstream_result.returncode),without_memfd_rc=fallback.returncode),indent=2))
