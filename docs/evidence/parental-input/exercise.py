...
generated=[launch_probe,keyboard_config,input_window,input_probe,settings_dir/'shortcut-approved',settings_dir/'shortcut-blocked']+a11y_probes+[settings_probe,raw_profile,settings_dir/'user',settings_dir/'elf-probe',bus_probe,window_test,launcher,dropin,profile,database,debuglog,keydir/'00-policy',keydir/'locks/00-policy']
...
backup = fixture_recovery.snapshot(
    input_services+[gio_launcher,worker,helper,native,ctl,shell,entry,application,xwayland,xclient,xkbcomp,mutter,x11ready,frames,xsettings,dconf_service,a11y_settings,a11y_hook]+atspi+[P('/etc/gdm/custom.conf')],
    generated+[P('/etc/pam.d/gdm-autologin'),P('/etc/gdm/custom.conf.lyra-baseline')], cache_directory=font_cache)
created_dirs=[]
loaded=False
audit_started=False
...
'''
    ...
    run(['chcon','-t','lyra_parental_probe_exec_t',str(input_probe),str(input_window)])
    ...
    with P('/root/trusted-shell-session.json').open('w') as log:
        subprocess.run(['python3','/root/restricted-xwayland-session.py'],check=True,stdout=log,timeout=220)
    import time
    time.sleep(1)
